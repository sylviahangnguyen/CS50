...
def buy():
    ...
    if request.method == "POST":
        ...
        total_price = request.form.get("shares") * stock_info["price"]
...
def quote():
    ...
    if request.method == "POST":
        ...
        if stock_info == None:
            return apology("invalid symbol", 403)
        return render_template("stock_info.html", name=stock_info["name"],
...
def register():
    ...
    if request.method == "POST":
        ...
        rows = db.execute("INSERT INTO users ( username, hash, cash) VALUES(:username, :hash_value, :cash)",
                          username=request.form.get("new_username"),
